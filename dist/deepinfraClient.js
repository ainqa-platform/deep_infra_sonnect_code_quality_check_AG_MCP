...
export async function analyzeCodeQuality(filePath, code) {
    const prompt = `
...
\`\`\`
`;
    const response = await fetch(`${CONFIG.deepinfraBaseUrl}/chat/completions`, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
        ...
        throw new Error(`DeepInfra API error: ${response.status} ${response.statusText} - ${text}`);
    }
...
//# sourceMappingURL=deepinfraClient.js.map